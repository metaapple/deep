...
# 2. 임베딩하고 ollama요청해서 답변받아오는 부분
    ...
    def generate(self, prompt: str) -> str:
        ...
        payload = {
            ...
            "stream": False,
            "options": {
                "temperature": 0.5, #네가 찾은 것 중에서 아주 정확한 것만!!
                "num_predict": 64,  
...
# 5. 질문하고 답변 만들어오는 부분
    ...
    def ask(self, question: str, top_k: int = 10) -> Dict[str, Any]:
        ...
        """
            ...
            return {"answer": answer, "chroma-db": docs, "mode": "크로마 db에서 추출됨"}

        # 2) 비밀키 같은 "패턴 추출" 질문이면: 정규식 extractive
        #    (질문에 '비밀키'가 들어갈 때만 동작하도록 유지)
        if "비밀키" in question:
            m = re.search(r"비밀키\D*([0-9]{2,})", merged)
            if m:
                return {"answer": m.group(1), "chroma-db": docs, "mode": "크로마 db에서 추출됨"}

        # 3) 그 외: 일반 RAG(generate)
        context = "\n\n---\n\n".join(docs)[:3000]
        prompt = f"""너는 문서 기반 QA 어시스턴트다.
        아래 CONTEXT에 있는 정보만 사용해서 질문에 답하라.
        없으면 '문서에서 찾을 수 없습니다.'라고 답하라.
        ...
        """
        answer = self.generate(prompt).strip()
        return {"answer": answer, "gemma3": docs, "mode": "generate"}
